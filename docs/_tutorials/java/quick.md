...
5. Change the server and the device information in _src/main/java/com/supremainc/sdk/example/quick/QuickStart.java_ as needed.
...
The first thing to do is to connect to the gateway and get a ___ManagedChannel___, which will be used for further communication. You have to know the IP address and port number of the gateway. And, you should also have the root certificate of the gateway for TLS/SSL communication. 
...
public class GrpcClient {
  ...
  // certFile is the pathname of the root certificate
...
public class ConnectSvc {
  ...
  public int connect(String deviceIP, int devicePort, boolean useSSL) throws Exception {
    ConnectInfo connInfo = ConnectInfo.newBuilder().setIPAddr(deviceIP).setPort(devicePort).setUseSSL(useSSL).build();
    ConnectRequest request = ConnectRequest.newBuilder().setConnectInfo(connInfo).build();
    ConnectResponse response = connectStub.connect(request);
...
2. Connect to the specified device. As default, the device is not set to use SSL. To use SSL, you have to enable it first using [Connect.EnableSSL]({{'/api/connect/' | relative_url}}#enablessl). The returned device ID will be used for other APIs.
    ...
    ```java
    int deviceID = connectSvc.connect(DEVICE_ADDR, DEVICE_PORT, false);
    ```
