...
4. Change the server and the device information in _src/example/quick/quickStart.go_ as needed.
...
1. Connect to the gateway and get a ___grpc.ClientConn___.
...
The first thing to do is to connect to the gateway and get a ___grpc.ClientConn___, which will be used for further communication. You have to know the IP address and port number of the gateway. And, you should also have the root certificate of the gateway for TLS/SSL communication. 
...
2. Connect to the gateway
    ...
    ```go
	  grpcClient.Connect(CA_FILE, SERVER_IP, SERVER_PORT)
    ```
...
func (s *ConnectSvc) Disconnect(deviceID uint32) error {
